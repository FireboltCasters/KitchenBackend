 ...
 */
const getUserProfile = async (userId, provider, database, env) => {
 ...
 */
const handleProviderLogin = async (shouldBeProvider, input, database, schema, services, env, hookUpdateUserInformation) => {
  ...
  if(!!hookUpdateUserInformation){ //check if hook function is provided
    ...
    if(currentProvider===shouldBeProvider){ //check if for our provider
      let existingUser = await database("directus_users") //get the user
          .where({ id: userId })
          .first();
      if(!!existingUser){
          let userProfile = await getUserProfile(userId, currentProvider, database, env); //get the remote user profile
    ...
    console.log("No hook function provided!");
  }
  return null;
}
